...
const getCurationFB = () => {
  return function (dispatch) {
    dispatch(loading(true));
    instance.get(`/curation`).then((res) => {
      console.log(res.data.curationlist);
...
const setUpdateFolderFB = (folderId, folder_name, status) => {
      ...
      if(res.data.ok){
        ...
        let _user_folder = user_folder.map(cur => {
          if(cur.folderId == folderId){
            return {...cur, folder_name: folder_name, folder_status: Number(status)}
          }
          return cur
        })
        ...
        if(history.location.pathname === '/mypage'){
          ...
          let folder_main = [{...folder_contents.maincuration[0], folder_name: folder_name, folder_status: Number(status)}]
